...
test('capturas de las pantallas', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Compilar' }).waitFor();
  await page.getByRole('banner').getByText(/^Artefacto/).waitFor();
  ...
  // Las pantallas con columnas de scroll propio se capturan con un viewport alto para que se vea todo.
  ...
  await page.screenshot({ path: join(OUT, 'generar.png') });
