...
import { EMPTY_FORM, buildAnalyzeRequest, buildGenerateRequest } from './form';
...
describe('formulario de Generar', () => {
  it('construye el cuerpo mínimo y el completo, sin campos vacíos ni opciones que no aplican', () => {
    ...
    expect(full).toEqual({ ok: true, body: { specialty: 'backend', offer: { text: 'Buscamos Kubernetes' }, format: 'pdf', engine: 'typst', theme: 'classic', locale: 'en', output: 'mi-cv.pdf', build: true, topN: 3, maxSkills: 0, maxProjects: 2, maxCertifications: 1, compact: true } });
