...
function fakeApi(overrides: Partial<ApiClient> = {}): ApiClient {
  return {
    ...
    validate: vi.fn(), build: vi.fn(), sources: vi.fn(), source: vi.fn(), writeSource: vi.fn(), generate: vi.fn(), analyze: vi.fn(), extractOffer: vi.fn(), themes: vi.fn(), createTheme: vi.fn(), installTheme: vi.fn(), verifyTheme: vi.fn(), outputs: vi.fn(), output: vi.fn(), exportProfile: vi.fn(), importProfile: vi.fn(), llmConfig: vi.fn(async () => LLM_CONFIG), writeLlmConfig: vi.fn(), checkLlm: vi.fn(), offerHistory: vi.fn(), shutdown: vi.fn(), reviews: vi.fn(), review: vi.fn(), writeReview: vi.fn(), deleteReview: vi.fn(), applyReview: vi.fn(),
    ...overrides,
  };
}
...
describe('Co-piloto', () => {
  ...
  it('un 409 abre el consentimiento y confirmar reenvía con el estimateId; un 403 avisa; un fallo del formulario no llama a la API', async () => {
    ...
    await fireEvent.change(screen.getByLabelText('Proveedor'), { target: { value: 'openai' } });
    await fireEvent.click(screen.getByRole('button', { name: 'Lanzar' }));
