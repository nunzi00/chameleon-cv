...
const STATUS = {
  ...
  llm: { config: undefined, configError: undefined, health: undefined, keys: {}, keysFile: '', allowedHosts: [], remote: undefined, usable: true },
...
function fakeApi(overrides: Partial<ApiClient> = {}): ApiClient {
  return {
    ...
    validate: vi.fn(), build: vi.fn(), sources: vi.fn(), source: vi.fn(), writeSource: vi.fn(), generate: vi.fn(), analyze: vi.fn(), extractOffer: vi.fn(), themes: vi.fn(), createTheme: vi.fn(), outputs: vi.fn(), output: vi.fn(), shutdown: vi.fn(), reviews: vi.fn(), review: vi.fn(), writeReview: vi.fn(), deleteReview: vi.fn(), applyReview: vi.fn(),
    ...overrides,
  };
}
...
describe('Co-piloto', () => {
  it('lanza un improve, muestra qué sale, sigue el progreso por SSE y ofrece abrir la revisión', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: 'Lanzar' }));
  ...
  it('un 409 abre el consentimiento y confirmar reenvía con el estimateId; un 403 avisa; un fallo del formulario no llama a la API', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('Tarea')).toBeTruthy());
    await fireEvent.change(screen.getByLabelText('Tarea'), { target: { value: 'summarize' } });
    await fireEvent.input(screen.getByLabelText('Proveedor'), { target: { value: 'openai' } });
    await fireEvent.click(screen.getByRole('button', { name: 'Lanzar' }));
    await waitFor(() => expect(screen.getByRole('dialog')).toBeTruthy());
    expect(screen.getByText('Coste estimado: 2 peticiones')).toBeTruthy();
    expect(screen.getByText('peticiones: 2')).toBeTruthy();
    await fireEvent.click(screen.getByRole('button', { name: 'Confirmar coste y enviar' }));
