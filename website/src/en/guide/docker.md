...
| Tag | What it is |
|---|---|
| `ghcr.io/nunzi00/chameleon-cv:1.27.0` | The exact version (the same as `cv --version` and as the tar.gz); it is what `compose.yml` pins. |
...
- **`docker run` without Compose**:
  `docker run --rm -v "$PWD/my-profile:/work" ghcr.io/nunzi00/chameleon-cv:1.27.0 --help`.
