...
| Etiqueta | Qué es |
|---|---|
| `ghcr.io/nunzi00/chameleon-cv:1.24.0` | La versión exacta (la misma que `cv --version` y que el tar.gz); es la que fija `compose.yml`. |
...
```bash
gh attestation verify oci://ghcr.io/nunzi00/chameleon-cv:1.24.0 --owner nunzi00
docker buildx imagetools inspect ghcr.io/nunzi00/chameleon-cv:1.24.0 --format '{{ json .SBOM }}'
docker buildx imagetools inspect ghcr.io/nunzi00/chameleon-cv:1.24.0 --format '{{ json .Provenance }}'
```
...
- **`docker run` sin Compose**: `docker run --rm -v "$PWD/my-profile:/work" ghcr.io/nunzi00/chameleon-cv:1.24.0 --help`.
