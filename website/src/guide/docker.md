...
| Etiqueta | Qué es |
|---|---|
| `ghcr.io/nunzi00/chameleon-cv:1.3.0` | La versión exacta (la misma que `cv --version` y que el tar.gz); es la que fija `compose.yml`. |
| `:1.3` · `:1` · `:latest` | Alias móviles a la última 1.3.x / 1.x / estable. Las prereleases (`1.2.0-rc.1`) no mueven ningún alias. |
...
```bash
gh attestation verify oci://ghcr.io/nunzi00/chameleon-cv:1.3.0 --owner nunzi00
docker buildx imagetools inspect ghcr.io/nunzi00/chameleon-cv:1.3.0 --format '{{ json .SBOM }}'
docker buildx imagetools inspect ghcr.io/nunzi00/chameleon-cv:1.3.0 --format '{{ json .Provenance }}'
```
...
- **`docker run` sin Compose**: `docker run --rm -v "$PWD/my-profile:/work" ghcr.io/nunzi00/chameleon-cv:1.3.0 --help`.
