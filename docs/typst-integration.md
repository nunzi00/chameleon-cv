...
- **T-3.3 (2026-08-28)**: entregada. `src/typst/releases.json` es el manifiesto de integridad (versión, URL base oficial y, por plataforma, fichero, SHA-256 y tamaño; los hashes se calcularon descargando los ocho assets oficiales y se validan con zod al cargar). `cv typst install`: única operación de red, iniciada por el usuario y anunciada; https obligatorio hasta la URL final, 96 MiB y 120 s de límite, temporal 0600, SHA-256 en streaming comparado antes de tocar nada (un fichero alterado se elimina), extracción con el `tar` del sistema en un directorio temporal, comprobación de `--version`, renombrado atómico al destino (`~/.cache/chameleon-cv/typst/<versión>/typst`, 0700) y limpieza garantizada de temporales en cualquier fallo. `cv typst status`: evalúa los candidatos en el orden de `locateTypst` y devuelve 0 solo si hay un binario utilizable. Verificado con una descarga real del release (17,5 MB) en una caché aislada y un CV renderizado después sin `--typst-path`.
- **T-3.4 (2026-08-28)**: entregada. `templates/typst/cv.typ` rediseñada como implementación de referencia (paleta y escala declaradas al principio; secciones en versalitas espaciadas sobre una regla fina —`smallcaps` conserva el texto en minúsculas, así que la extracción no cambia—; fechas alineadas a la derecha en la misma línea que el título mediante `grid`; ubicación e impacto en gris; skills en tabla de dos columnas; `sticky: true` en títulos de sección y de entrada; pie de página «nombre · n / total» solo cuando hay más de una página; sin justificación ni silabación). El diseño se iteró revisando el render a PNG (`typst compile --format png`). Aceptación: el round-trip se compara con un golden propio (`tests/fixtures/golden/cv-backend.typst.txt`) y, además, con la **equivalencia de palabras** frente al golden de pdfkit (la maquetación reordena líneas; el contenido es idéntico, salvo dos separadores «·» absorbidos por la alineación de fechas), más la paginación con pie. Documentación: README («Motor PDF de calidad editorial»: porqué, cómo, personalización) y `docs/plantillas-typst.md` (contrato de `StructuredView`, reglas del contenedor para plantillas propias, iteración con PNG, ejemplo mínimo).

> **Nota (2026-08-29, T-5.1)**: la plantilla de referencia `templates/typst/cv.typ` vive ahora en `themes/default/template.typ`, con sus variables de diseño en `themes/default/theme.toml` (sistema de temas, `--theme <nombre>`); el documento principal pasa a `#cv(vista, tema)`. Guía en `docs/plantillas-typst.md`.
