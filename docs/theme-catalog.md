...
Estado: APROBADA por el PO (2026-08-30, D1–D4) · Parte 1 (organizaciones) CERRADA el 2026-08-30 · Parte 2 (estilos) CERRADA el 2026-08-30 · Parte 3 (ampliación) IMPLEMENTADA el 2026-08-30
...
- **Parte 3 (2026-08-30)**: organizaciones `education-first`, `achievements-first` (panel de logros con el primero de cada puesto y su empresa) y `unified-timeline` (una fila por elemento con fecha —puesto, formación, certificación, proyecto— ordenada por la fecha ISO con `sorted(key:)`, etiqueta del apartado bajo el periodo, sin fecha al final); estilos `swiss` (títulos en la columna izquierda con `state`, acento rojo), `newspaper` (mancheta con doble filete, `columns(2)` en experiencia y proyectos, Libertinus) y `pastel`. Vista estructurada con `start`/`end`/`dateIso` (aditivo; PoC JSON regenerado). Pruebas: la común de los 27 (9 + 18), orden de secciones de las tres organizaciones con Typst real (formación antes que experiencia; logros con «— Nexo Pagos» al inicio; el máster de 2014–2015 entre Data Engineer y la etapa freelance), `cv theme list` (27 = 9 + 18), galería y seis generaciones más en el arnés `typst`. → 27 temas; versión 1.8.0.
