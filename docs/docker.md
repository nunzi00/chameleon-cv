...
| `ollama/ollama` v0.33.2: **3,2 GB comprimidos** (amd64); `qwen3:8b`: 5,2 GB. | La IA es opcional (`compose.ai.yml`): el perfil base no descarga nada de eso. |
...
- **Modelo**: `qwen3:8b` es el validado por defecto (arnés de IA 16/16, T-8.11; `qwen2.5:7b-instruct` también 16/16, más rápido pero con reescrituras rechazadas por C2). Otro modelo se recomienda solo tras pasar `npm run test:acceptance:ai` con él (C12/C13).
...
| S2 | `compose.yml`, `compose.ai.yml`, `compose.gpu.yml`, `.env.example`. | `docker compose config`; `run --rm` de todos los flujos base; superposición de IA con un doble de Ollama en loopback (como en el spike) y, una vez, con Ollama real y `qwen3:8b`. |
...
4. **Modelo `qwen3:8b`** (validado; `qwen2.5:7b-instruct` hasta 1.7.0) como preconfigurado (recomendado); cualquier otro solo tras el arnés de IA.
