...
| Tamaños por capa: base Debian 85 MB · `cv` **160 MB** · Typst 56 MB · avisos 0,3 MB → imagen ≈ 300 MB (distroless ≈ 240 MB). El `cv` de esta máquina pesa 72 MB porque el Node de Arch está *stripped*. | Aceptable para 1.x; queda registrado investigar `strip` del ejecutable SEA (§9). |
