// PoC T-3.1 (docs/typst-integration.md): plantilla Typst sobre el modelo de vista de Chameleon CV.
// Recibe un diccionario ya decodificado (JSON generado desde `CvView` + `inline.ts`: el Markdown
// en línea llega como «runs» con negrita/cursiva/código/enlace; los párrafos y listas como
// «blocks»). La plantilla no interpreta Markdown ni lee ficheros: solo maqueta datos.
