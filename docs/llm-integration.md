...
| **C11** | **Presupuesto y límites.** | Máximo de ítems por ejecución, `max_tokens`, tiempo por llamada (contenedor de red: 60 s), **estimación de tokens y aviso de coste antes de enviar a un remoto** (`src/llm/estimate.ts`; confirmación `s/N` o `--yes`); sin bucles de reintento ilimitados. |
...
- **Pendiente**: T-4.6 (`cv suggest tags`), T-4.7 (`cv improve apply`, opcional).
