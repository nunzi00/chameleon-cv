...
- **CSP** para la SPA: `default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' blob: data:; font-src 'self'; connect-src 'self'; frame-src blob:; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`. Sin *scripts* ni estilos en línea (Svelte compila los estilos a un `.css`; se evitan atributos `style`). `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer` (ya), `X-Content-Type-Options: nosniff` (ya).
...
- Pendiente de aprobación de esta propuesta.
