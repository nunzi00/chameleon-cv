...
| **Estado** | PROPUESTA v1 (2026-08-29) **APROBADA en su totalidad** por el Director de Ingeniería y Producto el 2026-08-29, con las ocho decisiones de §11. T-7.4a **aprobada** el 2026-08-29; T-7.4b en curso. |
...
| `POST /jobs/improve` · `/jobs/summarize` · `/jobs/suggest-tags` | Crean un **trabajo** con las mismas opciones que la CLI (sin rutas). Respuesta 202 `{ id, status }`. Con proveedor remoto sin consentimiento → 409 `consent-required` con la estimación de coste (§6). |
| `GET /jobs` · `GET /jobs/{id}` | Estado (`queued`, `running`, `done`, `failed`, `cancelled`), progreso (ítem actual / total, tiempos) y resultado (revisión escrita, estadísticas). |
| `GET /jobs/{id}/events` | SSE con las mismas líneas de progreso que la CLI imprime por stderr. |
| `DELETE /jobs/{id}` | Cancela (la petición en curso al modelo termina; no se escribe la revisión). |
...
| `PUT /reviews/{name}` ✎ | Guarda la revisión editada por el usuario (marcas `[x]`, retoques de texto) con `If-Match`. |
| `POST /reviews/{name}/apply` ✎ | `{ dryRun?, deleteReview? }` → lo mismo que `cv improve apply`: plan, ficheros escritos con su `.bak`, o 409 si no hay marcas o la huella cambió. |
