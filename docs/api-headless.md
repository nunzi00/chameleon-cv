...
| `POST /vida-laboral` | Compara las fechas de tus fuentes con el informe de vida laboral (cuerpo `application/pdf`, T-9.28) y devuelve los apuntes. No escribe nada y del informe **solo se leen empresas y fechas**: el DNI, el número de la Seguridad Social y el domicilio del PDF no se leen ni se guardan. |
| `POST /linkedin/plan` | El plan para poner LinkedIn al día desde tus fuentes (T-9.27): `add`, `fix` y `pending`. Con `draft`, compara con lo que exportaste de LinkedIn e importaste como borrador; sin él, solo dice qué tienes tú. No escribe nada y no sale de la máquina: es un diff local. |
| `DELETE /sources/{path}` ✎ · `POST /sources-delete-plan/{path}` | Borra un fichero de fuentes con `If-Match` (T-9.25); el plan dice antes **qué entradas del perfil desaparecen**, sin borrar nada. Se niega (422) si sin ese fichero el dataset deja de cargar, y la versión anterior queda en el histórico, así que `cv history restore` lo devuelve. |
| `PUT /reviews/{name}` ✎ · `DELETE /reviews/{name}` ✎ | Guarda la revisión editada por el usuario (marcas `[x]`, retoques de texto) con `If-Match`; elimina una revisión (esté a la vista o archivada). |
| `POST /reviews/{name}/apply` ✎ | `{ dryRun?, deleteReview?, archive? }` → lo mismo que `cv improve apply`: por defecto solo el plan (`dryRun: true`); con `dryRun: false`, ficheros escritos con la ruta de su versión anterior en el histórico (`output/historial-fuentes/<entrada>/<ruta>`, T-8.10) y la entrada creada (`history`); 422 con las líneas si no hay marcas o la huella cambió. Al terminar, la revisión que no deja **nada pendiente** se aparta sola (`archived`); `archive: false` lo evita. |
| `POST /reviews/{name}/archive` ✎ | `{ archived }` → aparta la revisión a `output/revisiones-archivadas/` o la devuelve a la vista (T-9.24). Mover, no borrar; idempotente (`moved: false` si ya era así); nunca sobrescribe. `GET /reviews` devuelve los dos grupos: `reviews` y `archived`. |
| `POST /reviews/{name}/undo` ✎ | Deshace **la última aplicación de esa revisión**: busca en el histórico la entrada `apply` cuyo `origin` es su nombre y devuelve cada fuente a como estaba, guardando a su vez lo que había (entrada `restore`: deshacer el deshacer es posible). Saca la revisión del archivo, porque vuelve a estar pendiente. 422 si el histórico no guarda ninguna aplicación suya. |
...
## §7 Estructura del registro de rutas (1-sep-2026)

`src/serve/routes.ts` tenía **cincuenta y una rutas en una sola función de casi novecientas líneas**, y encontrar
una era cuestión de suerte. Ahora el registro va **por dominios** —`addWorkspaceRoutes`, `addConfigRoutes`,
`addHistoryRoutes`, `addGenerateRoutes`, `addOfferRoutes`, `addImportRoutes`, `addThemeRoutes`,
`addServerRoutes` y el ya existente `addCopilotRoutes`—, todas sobre el mismo enrutador y **en el mismo orden de
registro de siempre**: lo aprovecha la referencia generada, y el arnés lo comprueba byte a byte (el listado de
rutas viaja en el golden de `serve`).

**El puente levadizo, en un solo sitio.** Toda salida a la red pasa por la misma regla en dos tiempos —403 si el
servidor no admite remotos, 409 con un `estimateId` de un solo uso mientras no se confirme— y estaba escrita
**cuatro veces**: los trabajos del co-piloto, la descarga de una oferta, la instalación de un tema y la lectura
de la oferta por el co-piloto. Cuatro copias de una regla de seguridad es justo donde se cuela una divergencia.
