...
1. **BuildKit** (`docker/build-push-action` con `sbom: true` y `provenance: mode=max`): el SBOM (SPDX) y la procedencia SLSA de la construcción viajan **dentro del registro** como manifiestos de atestación del índice; se leen con `docker buildx imagetools inspect ghcr.io/nunzi00/chameleon-cv:1.1.0 --format '{{ json .SBOM }}'` (o `.Provenance`).
