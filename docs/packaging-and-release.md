...
1. **verify** (`ubuntu-24.04`): `npm ci`, `npm run typecheck`, `npm run lint`, `npm run coverage` (umbral 100 %), y las pruebas con Typst real (`cv typst install` en el runner, con caché de Actions por versión) para que el golden del PDF se compruebe en cada release; comprobación tag = versión.
