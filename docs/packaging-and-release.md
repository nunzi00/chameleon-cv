...
- Notas de la release: la sección `## [<versión>] - <fecha>` de `CHANGELOG.md` («Keep a Changelog»), extraída con `npm run release:notes -- <versión>`; `verify` comprueba que existe, con fecha y contenido, antes de empaquetar (T-6.6; sustituye a la idea inicial de `--generate-notes` a partir de los commits).
...
3. **release** (`needs: build`, permisos `contents: write`, `id-token: write`, `attestations: write`): descarga de artefactos, `sha256sum * > SHA256SUMS.txt`, `actions/attest-build-provenance` (atestación SLSA firmada por Sigstore, verificable con `gh attestation verify`), `gh release create "$TAG" --verify-tag --notes-file RELEASE-NOTES.md` con los archivos y `SHA256SUMS.txt` adjuntos.
...
| **T-6.4** (opcional) | npm con *trusted publishing*; *tap* de Homebrew. **Pospuesto tras la v1.0.0** (Director, 2026-08-29). | `npm i -g chameleon-cv`, `brew install …` |
| **T-6.5** (decisión) | Firma/notarización macOS y Windows. **Pospuesto tras la v1.0.0** (Director, 2026-08-29): se abordará con esas plataformas. | Binarios firmados |
| **T-6.6** | 1.0.0: licencia MIT y avisos de terceros en el archivo, `CHANGELOG.md` como fuente de las notas, revisión final del README; el tag lo pone el Director (`cv --version` se mantiene sin cambios por la decisión 4 de T-6.2). | Release v1.0.0 |
...
- **T-6.6 (2026-08-29)**: entregada. **Licencia MIT** (`LICENSE`; `package.json` con `license` y `author`) y **avisos de licencias de terceros** en el archivo: el paso 6/7 de `scripts/package.ts` genera `THIRD-PARTY-NOTICES.md` a partir del `metafile` de esbuild —los 65 paquetes npm que de verdad viajan en los bundles (58 MIT, 2 Apache-2.0, 2 BSD-3-Clause, 1 ISC, 1 0BSD y 1 sin identificador pero con texto; 4 declaran MIT sin incluir el texto y se indica solo el identificador), la licencia de Node.js leída de la distribución que `--build-sea` embebe (junto al ejecutable en las distribuciones oficiales —`setup-node`, nvm, fnm—, `/usr/share/licenses/nodejs` en Arch, `/usr/share/doc/nodejs/copyright` en Debian, o `CHAMELEON_NODE_LICENSE`; sin ella el empaquetado se detiene) y las fuentes OFL—; el archivo lleva ahora `LICENSE`, `CHANGELOG.md`, `THIRD-PARTY-NOTICES.md` (247 KB) y `LICENSE-SourceSans3.md` (`src/release/notices.ts`, 100 % de cobertura). **`CHANGELOG.md`** («Keep a Changelog»; la 1.0.0 resume los Hitos 1–6) como única fuente de las notas de la release: `npm run release:notes -- <versión>` (`src/release/changelog.ts`) las extrae y exige sección con fecha ISO y contenido; `verify` lo comprueba antes de empaquetar, `package-linux-x64` las adjunta al artefacto (`RELEASE-NOTES.md`) y `release` publica con `--notes-file` en lugar de `--generate-notes`. **Pulido**: README («Qué hace», requisitos por vía de instalación, instalación desde *Releases* con `sha256sum -c` y `gh attestation verify`, opciones remotas en la tabla de comandos, sección «Licencia»), esta nota (§6, §8, estado) y el ROADMAP (T-6.4 y T-6.5 pospuestas tras la v1.0.0 por decisión del Director). **Verificación**: 620 tests con el 100 % de cobertura (Typst real incluido), arnés determinista 77/77 con `dist/` y 77/77 contra el ejecutable, `npm run package` en 20,8 s (`chameleon-cv-1.0.0-linux-x64.tar.gz`, 23 MB), `release:notes` con la 1.0.0 (52 líneas) y código 2 con una versión inexistente. **Versión 1.0.0** en `package.json`; el tag `v1.0.0` lo pone el Director. Lo que solo puede darse con el repositorio en GitHub: los campos `repository`/`homepage` de `package.json` y los enlaces de comparación del CHANGELOG (necesitan el nombre del repositorio), y la comprobación de que `setup-node` deja `LICENSE` junto al Node del runner (si no, `CHAMELEON_NODE_LICENSE` en el workflow). **APROBADA por el Director el 2026-08-29**; repositorio confirmado `lucasnunzi/chameleon-cv`: `repository`, `homepage` y `bugs` en `package.json`, enlaces de comparación del CHANGELOG, enlace a Releases, `--owner` y distintivo de CI en el README.
