...
- **T-7.1 (2026-08-29)**: entregada. `website/` con VitePress 1.6.4 fijado (173 paquetes en su propio lockfile), configuración en `website/.vitepress/config.mts` (castellano, búsqueda local, enlaces muertos como error, `base` desde `DOCS_BASE`, HTML crudo desactivado para que `<nombre>` sea texto, repositorio leído de `package.json` como única fuente de verdad), tema con la paleta de los CV y logotipo. **Contenido**: portada; guía de usuario en nueve capítulos (inicio rápido ejecutable, conceptos, formato de las fuentes, generar, ofertas, Typst y temas, co-piloto, seguridad y privacidad, solución de problemas); cuatro tutoriales ejecutables; sección de desarrolladores (visión arquitectónica con dos diagramas SVG y la tabla de cánones C1–C15, contribuir, pruebas, extender —formato de fuente, proveedor de modelos, tema, comando—, empaquetado y release, documentación como código, ROADMAP); notas de diseño, CHANGELOG y licencia. **Docs-as-Code** (`scripts/docs/`): `reference.ts` genera 21 páginas desde commander con ancho fijo y exige un fichero de ejemplos por comando hoja (`website/examples/`, 16); `sync.ts` publica las 15 notas de `docs/`, `CHANGELOG.md`, `CONTRIBUTING.md`, `ROADMAP.md` y `LICENSE` reescribiendo los enlaces (páginas del portal o GitHub; sin tocar los bloques de código) y **falla si algún enlace a GitHub no coincide con el repositorio de `package.json`**; `tutorials.ts` ejecuta los bloques `bash tutorial` de las cinco páginas con cabecera `verify:` en un espacio temporal con `cv` resuelto al binario (25 bloques: 22 sin requisitos y 3 `needs-llm`; los `needs-typst` corren con la caché de usuario) y comprueba los ficheros prometidos; omisiones visibles, nunca silenciosas. **Verificación**: `npm run docs:check` en verde (referencia 21/21, sincronización sin avisos, build de 63 páginas sin enlaces muertos en 6,7 s, tutoriales 5/5 con Typst real, 3 bloques omitidos por falta de modelo); **tutorial 4 ejecutado íntegro con Qwen2.5-7B** (llama-server local, `CHAMELEON_DOCS_LLM=1`): 25/25 bloques en 144 s, con una propuesta de `summarize` rechazada por el verificador C2 y `suggest tags --untagged` etiquetando el logro que el propio tutorial añade —las dos primeras ejecuciones destaparon errores del tutorial, no del producto (`--untagged` sin logros pendientes y `apply` sin marcas terminan con código 1 por diseño), corregidos enseñando el flujo correcto—; la guarda del repositorio probada con un enlace erróneo (lo detecta y falla); suite unitaria 620/620. **Despliegue**: `.github/workflows/pages.yml` (build en cada PR; build y despliegue en `main`; `DOCS_BASE` derivado de `GITHUB_REPOSITORY`; Typst en caché; acciones fijadas por SHA: configure-pages v6.0.0, upload-pages-artifact v5.0.0, deploy-pages v5.0.0), Dependabot para `website/`. **README** reducido a puerta corta (125 líneas) con enlaces al portal. **Desviaciones respecto a §5**: los scripts viven en `scripts/docs/` (typecheck y ts-node de la raíz) en lugar de `website/scripts/`; los diagramas son SVG escritos a mano (sin `mermaid-cli` ni fuentes `.mmd`); las barras laterales de la referencia y de las notas se generan como JSON. **Activación**: en el repositorio, *Settings → Pages → Source: GitHub Actions*; el primer push a `main` publica `https://nunzi00.github.io/chameleon-cv/`. Pendiente: T-7.1b (inglés).

## Rediseño del portal (T-8.6 S4, 2026-08-30)

- Tema: `website/.vitepress/theme/index.ts` extiende `vitepress/theme-without-fonts` (sin la fuente Inter descargada; solo fuentes del sistema) y aporta `Layout.vue`, que compone la portada con las ranuras del tema por defecto: `home-hero-info-before` (chip «Local y soberano · sin cuenta, sin telemetría»), `home-hero-actions-after` (los tres comandos) y `home-features-after` (`HomeBlocks.vue`: «Qué hace / Qué no hace», «Tres caminos» y «Temas» con miniaturas reales de `public/themes/`). La captura del hero es `public/gui/estado.png`, regenerada con `npm run gui:screenshots`.
- Tokens: `custom.css` mapea la paleta del sistema visual de la aplicación (`gui/src/app.css`) a `--vp-c-*` en claro y oscuro (pantallas.md §8) y pule las páginas interiores (§9). Sin `@import` ni `url(https…)`.
- Pruebas: `tests/docs/portal-tokens.test.ts` (AA ≥ 4,5:1 en once pares de color por modo; tipografía del sistema; tema sin fuentes) y `gui/e2e-portal/portal.spec.ts` sobre el sitio construido con `vitepress preview` (`npm run docs:e2e`; config `gui/playwright.portal.config.ts`): la portada responde «qué es y qué no hace» sin desplazarse en 1440×900, el modo oscuro mantiene AA en texto, marca y enlaces, y ninguna petición sale del sitio. El flujo de Pages las ejecuta tras `docs:check`.
- Markdown con `html: false` (las notas de diseño citan `<nombre>` en prosa): por eso los bloques de la portada van por ranuras del tema y no como componentes dentro de `index.md`.
