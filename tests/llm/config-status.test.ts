...
describe('llmStatus / formatLlmStatus', () => {
  ...
  it('un remoto pendiente de verificación sin nota se describe como no disponible', async () => {
    ...
    const withoutNote = { ...status, providers: status.providers.map((provider) => (provider.id === 'groq' ? { ...provider, availability: 'pending-verification' as const, availabilityNote: undefined } : provider)) };
  ...
  it('describe el proveedor local alcanzable, la procedencia de las claves y la lista blanca', async () => {
    ...
    expect(formatLlmStatus(status)).toBe(
      [
        ...
        '  gemini → clave ninguna · plan gratuito (límites según la cuenta) · generativelanguage.googleapis.com · modelo por defecto gemini-3.6-flash · AVISO: el plan gratuito de Gemini usa las peticiones para mejorar los productos de Google: no envíes lo que no quieras compartir',
        '    modelos (--model o [llm.models]): gemini-3.6-flash (estable) · gemini-2.5-flash (estable) · gemini-2.5-pro (estable)',
