...
describe('cv serve: GET/PUT /config/llm y POST /config/llm/check', () => {
  ...
  it('GET devuelve la configuración efectiva, cv.toml con su huella, los proveedores del registro sin claves y la cuota viva', async () => {
    ...
    expect(body.llm.config.sources).toEqual({ provider: 'default', baseUrl: 'default', model: 'default' });
  ...
  it('check comprueba el local por defecto, rechaza los remotos sin --allow-remote y con él explica la clave ausente o el id desconocido', async () => {
    ...
    const noKey = await post(remoteServer, '/config/llm/check', { provider: 'groq' });
    expect(noKey.status).toBe(200);
    expect(await noKey.json()).toMatchObject({ provider: 'groq', kind: 'remote', ok: false, message: expect.stringContaining('No hay clave para «groq»') as string });
    ...
    env = { CHAMELEON_GROQ_API_KEY: 'gsk' };
    const remoteMissing = await post(remoteServer, '/config/llm/check', { provider: 'groq' });
    expect(await remoteMissing.json()).toEqual({ provider: 'groq', kind: 'remote', ok: false, models: ['qwen', 'otro'], modelAvailable: false, message: 'el modelo configurado «openai/gpt-oss-120b» no está disponible', quota: { provider: 'groq', observedAt: '2026-08-30T12:00:00.000Z', remainingRequests: 28, limitRequests: 30 } });
    const remoteOk = await post(remoteServer, '/config/llm/check', { provider: 'groq', model: 'qwen' });
    expect(await remoteOk.json()).toMatchObject({ provider: 'groq', kind: 'remote', ok: true, modelAvailable: true });
    down = true;
    expect(await (await post(remoteServer, '/config/llm/check', { provider: 'groq' })).json()).toMatchObject({ provider: 'groq', kind: 'remote', ok: false, message: expect.stringContaining('ECONNREFUSED') as string });
