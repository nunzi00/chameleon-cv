...
const GALLERY_THEMES: readonly string[] = ['modern', 'academic', 'minimal'];
...
describe('temas distribuidos (T-8.3): contrato común', () => {
  it('los cinco temas cargan, validan, se describen y los de la galería declaran autor, licencia y página', async () => {
    const themes = await builtinThemes();
    expect(themes.map((theme) => theme.name)).toEqual(['academic', 'classic', 'default', 'minimal', 'modern']);
