...
/** Un PDF de sesenta páginas (límite de páginas), como el del corpus del spike pero autocontenido. */
function sixtyPagesPdf(): Promise<Buffer> {
  return new Promise((resolvePdf) => {
    const doc = new PDFDocument({ autoFirstPage: true });
    const chunks: Buffer[] = [];
    doc.on('data', (chunk: Buffer) => chunks.push(chunk));
    doc.on('end', () => resolvePdf(Buffer.concat(chunks)));
    doc.font(join(__dirname, '../../templates/fonts/SourceSans3-Regular.ttf')).fontSize(12);
...
import { extractItems } from '../../src/import/items';
