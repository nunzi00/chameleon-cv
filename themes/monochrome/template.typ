// Tema «monochrome» de Chameleon CV (T-8.12, estilo): solo negro y grises, pensado para imprimir y fotocopiar. Jerarquía por
// peso y tamaño, reglas negras gruesas bajo cada sección y enlaces en negro (sin color que se pierda en papel).
...
// Para personalizar: `cv theme create mio --from monochrome` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  ...
  let headline = d.at("headline", default: none)
  if headline != none { block(below: 0.4em, text(size: s.sizes.headline, weight: "semibold", fill: s.ink)[#headline]) }
  if d.contact.len() > 0 { block(below: 0.6em, text(size: s.sizes.contact, fill: s.muted, runs(d.contact))) }
  line(length: 100%, stroke: 1.6pt + s.rule)
  if d.summary.len() > 0 { v(0.5em); blocks(d.summary) }
}
...
#let title(s, t) = block(
  ...
  sticky: true,
  inset: (bottom: 3pt),
  stroke: (bottom: 1.2pt + s.rule),
  text(font: s.fonts.heading, size: s.sizes.section, weight: "bold", fill: s.primary, tracking: 0.08em, upper(t)),
)
