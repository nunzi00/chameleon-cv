// Tema «swiss» de Chameleon CV (T-8.12, estilo): estilo tipográfico internacional. Rejilla estricta con los títulos
// de sección en una columna izquierda en mayúsculas pequeñas, contenido alineado a la izquierda en la columna ancha,
// mucho aire, sin adornos y un único acento rojo en el nombre. Sans-serif y reglas finas.
...
// Para personalizar: `cv theme create mio --from swiss` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  block(below: 4pt, text(font: s.fonts.heading, size: s.sizes.name, weight: "bold", fill: s.accent, tracking: -0.02em)[#d.fullName])
  let headline = d.at("headline", default: none)
  if headline != none { block(below: 0.4em, text(size: s.sizes.headline, weight: "regular", fill: s.ink)[#headline]) }
  if d.contact.len() > 0 { block(below: 0.8em, text(size: s.sizes.contact, fill: s.muted, runs(d.contact))) }
  line(length: 100%, stroke: 0.5pt + s.rule)
  if d.summary.len() > 0 { v(0.6em); grid(columns: (LEFT, 1fr), column-gutter: 1.5em, [], blocks(d.summary)) }
}

// El título vive en la columna izquierda de la primera fila de cada apartado.
#let pending = state("swiss-title", none)
#let title(s, t) = pending.update(t)
#let caption(s) = context {
  let t = pending.get()
  if t != none { text(font: s.fonts.heading, size: s.sizes.section, weight: "bold", fill: s.primary, tracking: 0.06em, upper(t)) } else { [] }
}
#let consume() = pending.update(none)

#let row-of(s, left, body, top: 0.9em) = block(width: 100%, above: top, below: 0.2em, breakable: true, grid(columns: (LEFT, 1fr), column-gutter: 1.5em, left, body))

#let piece(s, d, heading, subtitle, item, location: none) = {
  row-of(s, caption(s), {
    block(sticky: true, below: 0.2em, grid(
      columns: (1fr, auto),
      column-gutter: 1em,
      align: (left, right),
      text(font: s.fonts.heading, size: s.sizes.title, weight: "bold", fill: s.ink)[#heading],
      text(size: s.sizes.meta, fill: s.muted)[#subtitle],
    ))
    if location != none { block(above: 0em, below: 0.25em, text(size: s.sizes.meta, fill: s.muted)[#location]) }
    blocks(item.summary)
    for a in item.achievements { achievement(s, a) }
    if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
  })
...
#let skills(s, d) = { row-of(s, caption(s), skills-grid(s, d)); consume() }
#let line-item(s, lhs, rhs) = { row-of(s, caption(s), row(s, lhs, rhs), top: 0.45em); consume() }
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    if d.achievements.len() > 0 {
      ...
      row-of(s, caption(s), { for a in d.achievements { achievement(s, a) } })
      consume()
    }
    ...
    if d.languages.len() > 0 {
      ...
      row-of(s, caption(s), d.languages.map(l => [#strong(l.name): #l.level]).join([ · ]))
      consume()
    }
  })
}
