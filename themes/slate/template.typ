// Tema «slate» de Chameleon CV (T-8.16, estilo): grises azulados fríos con los títulos de sección sobre una banda suave: sobrio, legible y sin estridencias, pensado para sectores clásicos.
...
// Para personalizar: `cv theme create mio --from slate` y edita theme.toml o template.typ (contrato en
...
#let section(s, title) = block(
  ...
  sticky: true,
  inset: (x: 6pt, y: 3pt),
  fill: s.rule.lighten(45%),
  text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary, tracking: 0.07em, upper(title)),
)
