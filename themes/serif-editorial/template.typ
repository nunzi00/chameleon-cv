// Tema «serif-editorial» de Chameleon CV (T-8.16, estilo): estilo editorial en serif: Libertinus para el cuerpo y los títulos, versalitas suaves en las secciones, cifras del periodo en cursiva y un filete fino bajo el nombre.
...
// Para personalizar: `cv theme create mio --from serif-editorial` y edita theme.toml o template.typ (contrato en
...
#let section(s, title) = block(
  ...
  text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary, tracking: 0.06em, smallcaps(title)),
)
...
#let dated(s, period, body) = block(
  ...
  below: 0.35em,
  breakable: true,
  grid(columns: (8.6em, 1fr), column-gutter: 1.1em, align: (left, left), text(size: s.sizes.meta, fill: s.muted, style: "italic")[#period], body),
)
