// Tema «two-column-dense» de Chameleon CV (T-8.16, organización): todo el cuerpo en dos columnas equilibradas
// (experiencia, proyectos, habilidades, formación, certificaciones, logros e idiomas), con el periodo bajo cada
// título. Cabe más en la misma página sin apretar el interlineado.
...
// Para personalizar: `cv theme create mio --from two-column-dense` y edita theme.toml o template.typ (contrato en
...
#let body-flow(s, d) = {
  if d.experience.len() > 0 {
    ...
    for item in d.experience {
      block(above: 0.8em, below: 0.3em, breakable: true, {
  ...
  if d.skillGroups.len() > 0 {
    ...
    for g in d.skillGroups { block(below: 0.4em, text(size: s.sizes.body)[#strong(g.label): #text(fill: s.muted)[#g.names]]) }
  }
  education(s, d)
  certifications(s, d)
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    header(s, d)
    v(0.2em)
    // Dos columnas equilibradas de arriba abajo: cabe más en la misma página sin apretar el interlineado.
    columns(2, gutter: 1.4em, body-flow(s, d))
  })
}
