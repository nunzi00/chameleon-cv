// Tema «achievements-first» de Chameleon CV (T-8.12, organización): el impacto primero. Un bloque inicial con los logros
// destacados del perfil y el primer logro de cada puesto (con la empresa como origen); después la cronología corta
// (una línea por puesto con su resumen y tecnologías), proyectos en una línea, habilidades, formación, certificaciones
// e idiomas. Para candidaturas a puestos de responsabilidad donde cuentan los resultados medibles.
...
// Para personalizar: `cv theme create mio --from achievements-first` y edita theme.toml o template.typ (contrato en
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    // 1. Impacto: logros del perfil y el primero de cada puesto, con la empresa.
    let bullets = ()
    for a in d.achievements { bullets.push(achievement(s, a)) }
    for item in d.experience {
      if item.achievements.len() > 0 { bullets.push(achievement(s, item.achievements.first(), suffix: item.company)) }
    }
    if bullets.len() > 0 {
      section(s, d.labels.achievements)
      block(width: 100%, fill: s.rule.lighten(65%), inset: 9pt, radius: 3pt, breakable: true, bullets.join())
    }

    // 2. Cronología corta: periodo, puesto, empresa, ubicación, resumen y tecnologías.
    if d.experience.len() > 0 {
      ...
      for item in d.experience {
        ...
        dated(s, item.period, {
          ...
          blocks(item.summary)
          if item.technologies != "" { text(size: s.sizes.meta, fill: s.muted)[#text(weight: "semibold")[#d.labels.technologies:] #item.technologies] }
        })
      }
    }
