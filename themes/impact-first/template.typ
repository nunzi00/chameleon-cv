// Tema «impact-first» de Chameleon CV (T-8.16, organización): una banda con las tres primeras cifras de impacto
// de los logros anclados (el campo «impact» que ya calcula la CLI) y, debajo, la cronología completa. Si ningún
// logro tiene impacto medido, la banda no se dibuja.
...
// Para personalizar: `cv theme create mio --from impact-first` y edita theme.toml o template.typ (contrato en
...
// Las tres primeras cifras de impacto de los logros anclados (el campo «impact» que ya calcula la CLI),
// con el logro que las respalda resumido a doce palabras: la portada del CV son los resultados.
#let highlights(d) = {
  let found = ()
  for item in d.experience {
    for a in item.achievements {
      let impact = a.at("impact", default: none)
      if impact != none and found.len() < 3 {
        let words = a.runs.map(r => r.text).join("").split(" ")
        let short = words.slice(0, calc.min(12, words.len())).join(" ")
        found.push((impact: impact, text: short + (if words.len() > 12 { "…" } else { "" })))
      }
    }
  }
  found
...
#let impact-band(s, items) = block(
  ...
  grid(
    ...
    ..items.map(item => block(
      ...
      radius: 3pt,
      {
        text(font: s.fonts.heading, size: s.sizes.title * 1.25, weight: "semibold", fill: s.accent)[#item.impact]
        linebreak()
        text(size: s.sizes.meta, fill: s.muted)[#item.text]
      },
    )),
  ),
)
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    header(s, d)
    let items = highlights(d)
    if items.len() > 0 { impact-band(s, items) }
