// Tema «compact-grid» de Chameleon CV (T-8.12, estilo): rejilla compacta de datos. Cada entrada en dos columnas (periodo y
// ubicación a la izquierda, contenido a la derecha), habilidades y formación en rejillas de dos columnas, tamaños
// contenidos y márgenes cortos: mucha información en poco papel sin llegar al folio único.
...
// Para personalizar: `cv theme create mio --from compact-grid` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  grid(
    ...
    column-gutter: 1.5em,
    align: (left, right),
    text(font: s.fonts.heading, size: s.sizes.name, weight: "semibold", fill: s.primary)[#d.fullName],
    {
      let headline = d.at("headline", default: none)
      if headline != none { align(right + horizon, text(size: s.sizes.headline, fill: s.muted)[#headline]) }
    },
  )
  if d.contact.len() > 0 { block(above: 0.35em, text(size: s.sizes.contact, fill: s.muted, runs(d.contact))) }
  v(0.3em)
  line(length: 100%, stroke: 0.6pt + s.rule)
  if d.summary.len() > 0 { v(0.4em); blocks(d.summary) }
}
...
#let title(s, t) = block(
  ...
  inset: (bottom: 2pt),
  stroke: (bottom: 0.5pt + s.rule),
  text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary, tracking: 0.06em, upper(t)),
)

// Entrada en dos columnas: periodo y ubicación a la izquierda; título y contenido a la derecha.
#let piece(s, d, heading, subtitle, item, location: none) = block(
  width: 100%,
  above: 0.75em,
  below: 0.25em,
  breakable: true,
  grid(
    ...
    column-gutter: 1em,
    {
      text(size: s.sizes.meta, weight: "semibold", fill: s.muted)[#subtitle]
      if location != none { linebreak(); text(size: s.sizes.meta, style: "italic", fill: s.muted)[#location] }
    },
    {
      block(sticky: true, below: 0.25em, text(font: s.fonts.heading, size: s.sizes.title, weight: "semibold", fill: s.primary)[#heading])
      blocks(item.summary)
      for a in item.achievements { achievement(s, a) }
      if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
    },
...
// Habilidades en dos columnas de pares «categoría: nombres».
#let skills(s, d) = grid(
  columns: (1fr, 1fr),
  column-gutter: 1.4em,
  row-gutter: 0.35em,
...
#let line-item(s, lhs, rhs) = block(below: 0.3em, grid(columns: (8.2em, 1fr), column-gutter: 1em, text(size: s.sizes.meta, weight: "semibold", fill: s.muted)[#rhs], lhs))
