// Tema «newspaper» de Chameleon CV (T-8.12, estilo): aire de periódico. Cabecera centrada tipo mancheta con doble
// filete, serif Libertinus, y la experiencia y los proyectos compuestos a dos columnas con títulos en versalitas;
// el resto a una columna. El texto se extrae columna a columna en orden. Para perfiles de comunicación, edición,
// investigación o quien quiera un CV con carácter.
...
// Para personalizar: `cv theme create mio --from newspaper` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  line(length: 100%, stroke: 1.2pt + s.ink)
  v(-0.5em)
  line(length: 100%, stroke: 0.4pt + s.ink)
  align(center, {
    block(above: 0.7em, below: 0.3em, text(font: s.fonts.heading, size: s.sizes.name, weight: "bold", fill: s.primary)[#d.fullName])
    let headline = d.at("headline", default: none)
    if headline != none { block(below: 0.35em, text(size: s.sizes.headline, style: "italic", fill: s.muted)[#headline]) }
    if d.contact.len() > 0 { block(below: 0.6em, text(size: s.sizes.contact, fill: s.muted, runs(d.contact))) }
  })
  line(length: 100%, stroke: 0.4pt + s.ink)
  v(-0.5em)
  line(length: 100%, stroke: 1.2pt + s.ink)
  if d.summary.len() > 0 { v(0.7em); blocks(d.summary) }
}
...
#let title(s, t) = block(
  ...
  below: 0.6em,
  sticky: true,
  align(center, text(font: s.fonts.heading, size: s.sizes.section, weight: "bold", fill: s.primary, tracking: 0.08em, smallcaps(t))),
)

#let piece(s, d, heading, subtitle, item, location: none) = {
  block(sticky: true, above: 0.8em, below: 0.25em, {
    text(font: s.fonts.heading, size: s.sizes.title, weight: "bold", fill: s.primary)[#heading]
    linebreak()
    text(size: s.sizes.meta, style: "italic", fill: s.muted)[#subtitle#if location != none [ · #location]]
  })
  blocks(item.summary)
  for a in item.achievements { achievement(s, a) }
  if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
...
#let line-item(s, lhs, rhs) = block(below: 0.4em, row(s, lhs, rhs))
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    if d.languages.len() > 0 {
      ...
      align(center, d.languages.map(l => [#strong(l.name): #l.level]).join([ · ]))
    }
  })
}
