// Tema «default» de Chameleon CV (diseño T-3.4; tematizable desde T-5.1): la implementación de referencia
// de un CV de calidad editorial. Recibe la StructuredView (src/renderers/structured) ya decodificada —el
// Markdown en línea llega como «runs» (negrita, cursiva, código, enlace) y los párrafos y listas como
// «blocks»— y el tema (theme.toml, validado por la CLI: colores, fuentes, tamaños, espaciados y página).
// No interpreta Markdown, no lee ficheros ni importa paquetes: solo maqueta datos. La CLI genera el
// documento principal:
//   #import "/template.typ": cv
//   #cv(json(bytes("…vista…")), json(bytes("…tema…")))
//
// Para personalizar: copia este directorio a themes/<mi-tema>/ en tu proyecto; edita theme.toml (sin tocar
// código) o template.typ (maquetación) y genera con `cv generate-cv --format pdf --engine typst --theme mi-tema`
// (contrato en docs/plantillas-typst.md).
