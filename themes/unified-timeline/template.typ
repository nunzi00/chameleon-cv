// Tema «unified-timeline» de Chameleon CV (T-8.12, organización): un solo eje temporal. Puestos, formación,
// certificaciones y proyectos con fecha se ordenan juntos de más reciente a más antiguo (por las fechas ISO de la
// vista), cada uno con la etiqueta de su apartado bajo el periodo; lo que no tiene fecha va al final. Después,
// habilidades, logros e idiomas. Para trayectorias en las que estudiar y trabajar se solapan.
...
// Para personalizar: `cv theme create mio --from unified-timeline` y edita theme.toml o template.typ (contrato en
...
// Cada elemento con fecha se convierte en una fila homogénea: clave de orden (fecha ISO), etiqueta, periodo y contenido.
#let entries(s, d) = {
  let rows = ()
  for item in d.experience {
    rows.push((key: item.start, kind: d.labels.experience, period: item.period, body: {
  ...
  for e in d.education {
    let field = e.at("field", default: none)
    let degree = if field != none { e.degree + " (" + field + ")" } else { e.degree }
    rows.push((key: e.at("start", default: ""), kind: d.labels.education, period: e.period, body: [#strong(degree) · #e.institution]))
  }
  for c in d.certifications {
    let issuer = c.at("issuer", default: none)
    let url = c.at("url", default: none)
    rows.push((key: c.at("dateIso", default: ""), kind: d.labels.certifications, period: c.date, body: [#strong(c.name)#if issuer != none [ · #issuer]#if url != none [ · #link(url)[#d.labels.link]]]))
  }
  for item in d.projects {
    rows.push((key: item.at("start", default: ""), kind: d.labels.projects, period: item.meta, body: {
  ...
  // Más reciente primero; los elementos sin fecha (clave vacía) quedan al final en el orden del perfil.
  rows.filter(r => r.key != "").sorted(key: r => r.key).rev() + rows.filter(r => r.key == "")
}
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    let rows = entries(s, d)
    if rows.len() > 0 {
      section(s, d.labels.experience + " · " + d.labels.education + " · " + d.labels.projects)
      for r in rows {
        dated(s, {
          text(size: s.sizes.meta, weight: "semibold", fill: s.muted)[#r.period]
          linebreak()
          text(size: s.sizes.footer, fill: s.accent, smallcaps(r.kind))
        }, r.body)
      }
    }
