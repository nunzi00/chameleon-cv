// Tema «mono-grid» de Chameleon CV (T-8.16, estilo): monoespaciada de arriba abajo con la rejilla a la vista: títulos entre corchetes, periodos alineados y viñetas con guion, con aire de terminal.
...
// Para personalizar: `cv theme create mio --from mono-grid` y edita theme.toml o template.typ (contrato en
...
#let section(s, title) = block(
  ...
  text(font: s.fonts.heading, size: s.sizes.section, weight: "bold", fill: s.accent)[\[ #upper(title) \]],
)
...
#let headline-of(s, title, subtitle, location: none) = {
  let parts = (text(font: s.fonts.heading, size: s.sizes.title, weight: "bold", fill: s.primary)[#title],)
...
#let setup(s, d, body, footer: true) = {
  ...
  set list(marker: text(fill: s.muted)[–], indent: 0em, body-indent: 0.55em, spacing: s.spacing.list)
...
#let header(s, d) = {
  block(below: 8pt, text(font: s.fonts.heading, size: s.sizes.name, weight: "regular", fill: s.primary, tracking: 0em)[#d.fullName])
