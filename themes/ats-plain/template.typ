// Tema «ats-plain» de Chameleon CV (T-8.16, organización): una columna, sin filetes, sin tablas, sin versalitas y
// sin espaciado entre letras; títulos en mayúsculas y negrita, una sola familia tipográfica y el orden que esperan
// los lectores automáticos (experiencia, formación, certificaciones, proyectos, habilidades, logros, idiomas).
...
// Para personalizar: `cv theme create mio --from ats-plain` y edita theme.toml o template.typ (contrato en
...
// Título de sección en una línea, sin filetes, sin versalitas y sin espaciado entre letras: lo que mejor
// digieren los lectores automáticos de CV (ATS). Nada de tablas ni de columnas.
#let plain-section(s, title) = block(
  width: 100%,
  above: 1.1em,
  below: 0.4em,
  sticky: true,
  text(font: s.fonts.heading, size: s.sizes.section, weight: "bold", fill: s.ink)[#upper(title)],
)

#let plain-entry(s, title, subtitle, period, location: none) = block(above: 0.7em, below: 0.25em, sticky: true, {
  text(font: s.fonts.heading, size: s.sizes.title, weight: "bold", fill: s.ink)[#title]
  if subtitle != none { text(size: s.sizes.title)[ — #subtitle] }
  linebreak()
  text(size: s.sizes.meta, fill: s.muted)[#period#if location != none [ · #location]]
})

#let cv(d, theme) = {
  let s = styles(theme)
  setup(s, d, {
    block(below: 0.35em, text(font: s.fonts.heading, size: s.sizes.name, weight: "bold", fill: s.ink)[#d.fullName])
    let headline = d.at("headline", default: none)
    if headline != none { block(below: 0.35em, text(size: s.sizes.headline)[#headline]) }
    if d.contact.len() > 0 { block(below: 0.6em, text(size: s.sizes.contact, runs(d.contact))) }
    if d.summary.len() > 0 { v(0.15em); blocks(d.summary) }

    if d.experience.len() > 0 {
      plain-section(s, d.labels.experience)
      for item in d.experience {
        plain-entry(s, item.role, item.company, item.period, location: item.at("location", default: none))
        blocks(item.summary)
        for a in item.achievements { achievement(s, a) }
        if item.technologies != "" { block(above: 0.4em, text(size: s.sizes.meta)[#strong(d.labels.technologies): #item.technologies]) }
      }
    }

    if d.education.len() > 0 {
      plain-section(s, d.labels.education)
      for e in d.education {
        let field = e.at("field", default: none)
        let degree = if field != none { e.degree + " (" + field + ")" } else { e.degree }
        plain-entry(s, degree, e.institution, e.period)
      }
    }

    if d.certifications.len() > 0 {
      plain-section(s, d.labels.certifications)
      for c in d.certifications {
        let issuer = c.at("issuer", default: none)
        plain-entry(s, c.name, issuer, c.date)
      }
    }

    if d.projects.len() > 0 {
      plain-section(s, d.labels.projects)
      for item in d.projects {
        plain-entry(s, item.name, item.at("role", default: none), item.meta)
        blocks(item.summary)
        for a in item.achievements { achievement(s, a) }
        if item.technologies != "" { block(above: 0.4em, text(size: s.sizes.meta)[#strong(d.labels.technologies): #item.technologies]) }
      }
    ...
    if d.skillGroups.len() > 0 {
      plain-section(s, d.labels.skills)
      for g in d.skillGroups { block(below: 0.3em, text(size: s.sizes.body)[#strong(g.label): #g.names]) }
    }

    if d.achievements.len() > 0 {
      plain-section(s, d.labels.achievements)
      for a in d.achievements { achievement(s, a) }
    }

    if d.languages.len() > 0 {
      plain-section(s, d.labels.languages)
      for l in d.languages { block(below: 0.25em, text(size: s.sizes.body)[#strong(l.name): #l.level]) }
    }
  })
}
