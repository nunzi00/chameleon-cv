// Tema «bold» de Chameleon CV (T-8.12, estilo): titulares grandes de color. Nombre en acento a gran tamaño con una barra
// gruesa a la izquierda, secciones en mayúsculas grandes con subrayado grueso y puestos en negrita destacada. Para perfiles
// de producto, marketing, ventas o diseño que quieren presencia en la primera página.
...
// Para personalizar: `cv theme create mio --from bold` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  grid(
    columns: (6pt, 1fr),
    column-gutter: 12pt,
    grid.cell(fill: s.accent)[],
    {
      block(below: 6pt, text(font: s.fonts.heading, size: s.sizes.name, weight: "bold", fill: s.accent, tracking: -0.02em)[#d.fullName])
      let headline = d.at("headline", default: none)
      if headline != none { block(below: 0.4em, text(size: s.sizes.headline, weight: "semibold", fill: s.primary)[#headline]) }
      if d.contact.len() > 0 { block(below: 0.2em, text(size: s.sizes.contact, fill: s.muted, runs(d.contact))) }
    },
  )
  if d.summary.len() > 0 { v(0.8em); blocks(d.summary) }
}

// Título grande en mayúsculas con subrayado grueso de acento.
#let title(s, t) = block(
  width: 100%,
  above: 1.4em,
  below: 0.7em,
  sticky: true,
  {
    text(font: s.fonts.heading, size: s.sizes.section, weight: "bold", fill: s.primary, tracking: 0.05em, upper(t))
    v(-0.35em)
    line(length: 2.4em, stroke: 2.5pt + s.accent)
  },
)

#let piece(s, d, heading, subtitle, item, location: none) = {
  block(sticky: true, above: 1em, below: 0.35em, grid(
    columns: (1fr, auto),
    column-gutter: 1.2em,
    align: (left, right),
    text(font: s.fonts.heading, size: s.sizes.title, weight: "bold", fill: s.primary)[#heading],
    box(fill: s.rule, radius: 3pt, inset: (x: 5pt, y: 2.5pt), text(size: s.sizes.meta, weight: "semibold", fill: s.muted)[#subtitle]),
  ))
  if location != none { block(above: 0em, below: 0.3em, text(size: s.sizes.meta, fill: s.muted)[#location]) }
  blocks(item.summary)
  for a in item.achievements { achievement(s, a) }
  if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
}
