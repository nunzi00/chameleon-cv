// Tema «europass-like» de Chameleon CV (T-8.12, estilo): estructura tabular al estilo europeo. Columna izquierda estrecha
// con el nombre de cada apartado y el periodo de cada entrada, columna derecha con el contenido; azul institucional y
// filetes finos. Familiar para procesos públicos y europeos; el texto se extrae en orden (etiqueta, periodo, contenido).
...
// Para personalizar: `cv theme create mio --from europass-like` y edita theme.toml o template.typ (contrato en
...
// ── Piezas propias del estilo ─────────────────────────────────────────────────────────────────────
#let LEFT = 9.2em

// Fila de la tabla: etiqueta (azul, versalitas) a la izquierda y contenido a la derecha, con filete inferior fino.
#let tabular(s, label, body, top: 0.6em) = block(
  width: 100%,
  above: top,
  below: 0em,
  breakable: true,
  inset: (bottom: 5pt),
  stroke: (bottom: 0.4pt + s.rule),
  grid(columns: (LEFT, 1fr), column-gutter: 1.2em, label, body),
)

#let caption-of(s, t) = text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary, tracking: 0.05em, smallcaps(t))
#let meta-label(s, t) = text(size: s.sizes.meta, weight: "semibold", fill: s.muted)[#t]

#let head(s, d) = {
  tabular(s, [], {
    block(below: 4pt, text(font: s.fonts.heading, size: s.sizes.name, weight: "semibold", fill: s.primary)[#d.fullName])
    let headline = d.at("headline", default: none)
    if headline != none { block(below: 0.35em, text(size: s.sizes.headline, fill: s.ink)[#headline]) }
    if d.contact.len() > 0 { text(size: s.sizes.contact, fill: s.muted, runs(d.contact)) }
  }, top: 0em)
  if d.summary.len() > 0 { tabular(s, [], blocks(d.summary)) }
}

// El título de sección vive en la columna izquierda de la primera fila del apartado: se guarda hasta la primera entrada.
#let pending = state("europass-title", none)
#let title(s, t) = pending.update(t)
#let take-title(s) = context {
  let t = pending.get()
  if t != none { caption-of(s, t) } else { [] }
}
#let consume() = pending.update(none)

#let piece(s, d, heading, subtitle, item, location: none) = {
  tabular(s, {
    take-title(s)
    v(0.15em)
    meta-label(s, subtitle)
    if location != none { linebreak(); text(size: s.sizes.meta, style: "italic", fill: s.muted)[#location] }
  }, {
    block(sticky: true, below: 0.25em, text(font: s.fonts.heading, size: s.sizes.title, weight: "semibold", fill: s.ink)[#heading])
    blocks(item.summary)
    for a in item.achievements { achievement(s, a) }
    if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
  })
...
#let skills(s, d) = {
  tabular(s, take-title(s), skills-grid(s, d))
  consume()
}

#let line-item(s, lhs, rhs) = {
  tabular(s, { take-title(s); v(0.15em); meta-label(s, rhs) }, lhs, top: 0.35em)
  consume()
}

#let cv(d, theme) = {
  let s = styles(theme)
  setup(s, d, footer: true, {
    head(s, d)

    if d.experience.len() > 0 {
      title(s, d.labels.experience)
      for item in d.experience {
        piece(s, d, item.role + " · " + item.company, item.period, item, location: item.at("location", default: none))
      }
    }

    if d.projects.len() > 0 {
      title(s, d.labels.projects)
      for item in d.projects {
        let role = item.at("role", default: none)
        piece(s, d, if role != none { item.name + " · " + role } else { item.name }, item.meta, item)
      }
    }

    if d.skillGroups.len() > 0 {
      title(s, d.labels.skills)
      skills(s, d)
    }

    if d.achievements.len() > 0 {
      title(s, d.labels.achievements)
      tabular(s, take-title(s), { for a in d.achievements { achievement(s, a) } })
      consume()
    }

    if d.education.len() > 0 {
      title(s, d.labels.education)
      for e in d.education {
        let field = e.at("field", default: none)
        let degree = if field != none { e.degree + " (" + field + ")" } else { e.degree }
        line-item(s, [#strong(degree) · #e.institution], e.period)
      }
    }

    if d.certifications.len() > 0 {
      title(s, d.labels.certifications)
      for c in d.certifications {
        let issuer = c.at("issuer", default: none)
        let url = c.at("url", default: none)
        line-item(s, [#strong(c.name)#if issuer != none [ · #issuer]#if url != none [ · #link(url)[#d.labels.link]]], c.date)
      }
    }

    if d.languages.len() > 0 {
      title(s, d.labels.languages)
      tabular(s, take-title(s), d.languages.map(l => [#strong(l.name): #l.level]).join([ · ]))
      consume()
    }
  })
}
