// Tema «warm» de Chameleon CV (T-8.12, estilo): paleta cálida (terracota, ocre y crema). Cabecera sobre un panel crema,
// títulos de sección como etiquetas redondeadas con fondo suave y periodos en pastillas; cercano y legible, para perfiles
// de personas, educación, salud, hostelería o comunicación.
...
// Para personalizar: `cv theme create mio --from warm` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  block(width: 100%, fill: s.rule.lighten(45%), inset: (x: 14pt, y: 12pt), radius: 6pt, below: 0.8em, {
    block(below: 5pt, text(font: s.fonts.heading, size: s.sizes.name, weight: "semibold", fill: s.primary)[#d.fullName])
    let headline = d.at("headline", default: none)
    if headline != none { block(below: 0.35em, text(size: s.sizes.headline, fill: s.accent, weight: "semibold")[#headline]) }
    if d.contact.len() > 0 { text(size: s.sizes.contact, fill: s.muted, runs(d.contact)) }
  })
  if d.summary.len() > 0 { blocks(d.summary) }
}

// Etiqueta redondeada con fondo cálido.
#let title(s, t) = block(
  above: 1.3em,
  below: 0.6em,
  sticky: true,
  box(fill: s.rule, radius: 999pt, inset: (x: 10pt, y: 4pt), text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary, tracking: 0.04em)[#t]),
)

#let piece(s, d, heading, subtitle, item, location: none) = {
  block(sticky: true, above: 0.95em, below: 0.35em, grid(
    columns: (1fr, auto),
    column-gutter: 1.2em,
    align: (left, right),
    text(font: s.fonts.heading, size: s.sizes.title, weight: "semibold", fill: s.primary)[#heading],
    box(fill: s.rule.lighten(35%), radius: 999pt, inset: (x: 7pt, y: 2.5pt), text(size: s.sizes.meta, fill: s.accent, weight: "semibold")[#subtitle]),
  ))
  if location != none { block(above: 0em, below: 0.3em, text(size: s.sizes.meta, style: "italic", fill: s.muted)[#location]) }
  blocks(item.summary)
  for a in item.achievements { achievement(s, a) }
  if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
}

#let skills(s, d) = skills-grid(s, d)
#let line-item(s, lhs, rhs) = block(below: 0.45em, row(s, lhs, rhs))
