// Tema «hybrid» de Chameleon CV (T-8.12, organización): CV híbrido o combinado. Resumen y competencias clave arriba,
// en un panel sombreado que se lee en diez segundos; después la cronología completa (experiencia con logros, proyectos,
// logros, formación, certificaciones e idiomas). La organización más habitual para perfiles técnicos con recorrido.
...
// Para personalizar: `cv theme create mio --from hybrid` y edita theme.toml o template.typ (contrato en
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    // Competencias clave: panel sombreado antes de la cronología.
    if d.skillGroups.len() > 0 {
      section(s, d.labels.skills)
      block(width: 100%, fill: s.rule.lighten(65%), inset: 9pt, radius: 3pt, breakable: true, skills-grid(s, d))
    }
