// Tema «skills-first» de Chameleon CV (T-8.12, organización): la matriz de competencias primero. Una tabla por categoría
// con cada skill, su nivel y sus años (cuando constan en el perfil); después la experiencia completa, los proyectos, los
// logros, la formación, las certificaciones y los idiomas. Para ofertas que llegan como lista de requisitos y para
// cribas que buscan tecnologías concretas antes que trayectorias.
...
// Para personalizar: `cv theme create mio --from skills-first` y edita theme.toml o template.typ (contrato en
...
// Una fila por skill; la categoría solo en la primera de su grupo. Sin nivel ni años, se imprime un guion.
#let matrix(s, d) = {
  let head(t) = text(weight: "semibold", size: s.sizes.meta, fill: s.muted, tracking: 0.06em, smallcaps(t))
  let rows = ()
  for g in d.skillGroups {
    let first = true
    for item in g.items {
      let level = item.at("level", default: none)
      let years = item.at("years", default: none)
      rows.push(if first { text(weight: "semibold", fill: s.primary)[#g.label] } else { [] })
      rows.push([#item.name])
      rows.push(text(fill: s.muted)[#if level != none { d.labels.levels.at(level) } else { "—" }])
      rows.push(text(fill: s.muted)[#if years != none { str(years) } else { "—" }])
      first = false
    }
  }
  table(
    ...
    fill: (x, y) => if y == 0 { s.rule.lighten(40%) } else if calc.even(y) { s.rule.lighten(82%) } else { none },
    table.header(table.cell(colspan: 2, head(d.labels.skills)), head(d.labels.level), head(d.labels.years)),
    ..rows,
  )
}
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    // La cabecera de la tabla hace de título de sección (así «Habilidades» sale una sola vez).
