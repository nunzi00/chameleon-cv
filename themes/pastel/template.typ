// Tema «pastel» de Chameleon CV (T-8.12, estilo): paleta pastel (lavanda, menta y crema). Cabecera sobre un panel
// lavanda con esquinas redondeadas, títulos de sección en cajas de color suave, periodos en pastillas menta y filetes
// muy claros. Amable y actual, para perfiles creativos, de educación, atención al cliente o primeros empleos.
...
// Para personalizar: `cv theme create mio --from pastel` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  block(width: 100%, fill: s.rule, inset: (x: 14pt, y: 12pt), radius: 8pt, below: 0.9em, {
...
#let title(s, t) = block(
  ...
  below: 0.6em,
  sticky: true,
  box(fill: s.rule, radius: 5pt, inset: (x: 9pt, y: 4pt), text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary)[#t]),
)
...
#let piece(s, d, heading, subtitle, item, location: none) = {
  block(sticky: true, above: 0.95em, below: 0.35em, grid(
    ...
    align: (left, right),
    text(font: s.fonts.heading, size: s.sizes.title, weight: "semibold", fill: s.primary)[#heading],
    box(fill: s.accent.lighten(72%), radius: 999pt, inset: (x: 7pt, y: 2.5pt), text(size: s.sizes.meta, fill: s.accent.darken(25%), weight: "semibold")[#subtitle]),
  ))
