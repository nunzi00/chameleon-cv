// Tema «gazette» de Chameleon CV (T-8.16, estilo): aire de prensa clásica: nombre entre dos filetes, antetítulo centrado y secciones con doble regla fina; serif en los títulos y sans en el cuerpo.
// Mantiene la organización cronológica (el cuerpo de «chronological») y cambia solo la voz tipográfica.
...
// Para personalizar: `cv theme create mio --from gazette` y edita theme.toml o template.typ (contrato en
...
#let section(s, title) = block(
  ...
  sticky: true,
  inset: (y: 2.5pt),
  stroke: (top: 0.4pt + s.rule, bottom: 0.4pt + s.rule),
  text(font: s.fonts.heading, size: s.sizes.section, weight: "semibold", fill: s.primary, upper(title)),
)
...
#let header(s, d) = {
  line(length: 100%, stroke: 1.4pt + s.ink)
  v(0.35em)
  block(below: 0.3em, align(center, text(font: s.fonts.heading, size: s.sizes.name, weight: "semibold", fill: s.primary)[#d.fullName]))
  let headline = d.at("headline", default: none)
  if headline != none {
    block(below: 0.45em, align(center, text(font: s.fonts.heading, size: s.sizes.headline, fill: s.muted)[#upper(headline)]))
  }
  line(length: 100%, stroke: 0.5pt + s.ink)
  if d.contact.len() > 0 { v(0.4em); block(below: 0.6em, align(center, text(size: s.sizes.contact, fill: s.muted, runs(d.contact)))) }
  if d.summary.len() > 0 { v(0.2em); blocks(d.summary) }
}
  ...
  setup(s, d, {
    ...
    if d.education.len() > 0 {
      ...
      for e in d.education {
        let field = e.at("field", default: none)
        let degree = if field != none { e.degree + " (" + field + ")" } else { e.degree }
        dated(s, e.period, [#strong(degree) · #e.institution])
      }
    }
    ...
    if d.certifications.len() > 0 {
      ...
      for c in d.certifications {
        let issuer = c.at("issuer", default: none)
        let url = c.at("url", default: none)
        dated(s, c.date, [#strong(c.name)#if issuer != none [ · #issuer]#if url != none [ · #link(url)[#d.labels.link]]])
      }
    }
    ...
    if d.projects.len() > 0 {
      ...
      for item in d.projects {
        block(above: 0.9em, below: 0.35em, breakable: true, {
          headline-of(s, item.name, item.at("role", default: none))
          block(below: 0.35em, text(size: s.sizes.meta, fill: s.muted)[#item.meta])
          blocks(item.summary)
          for a in item.achievements { achievement(s, a) }
          if item.technologies != "" { technologies(s, d.labels.technologies, item.technologies) }
        })
