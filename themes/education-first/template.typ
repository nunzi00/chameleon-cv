// Tema «education-first» de Chameleon CV (T-8.12, organización): la formación primero. Formación y certificaciones
// abren el documento, después habilidades y proyectos (donde un perfil junior demuestra lo que sabe hacer) y por
// último la experiencia completa, los logros y los idiomas. Para recién titulados, becas, prácticas y cambios de
// carrera con formación reciente.
...
// Para personalizar: `cv theme create mio --from education-first` y edita theme.toml o template.typ (contrato en
  ...
  setup(s, d, {
    ...
    if d.education.len() > 0 {
      ...
      for e in d.education {
        let field = e.at("field", default: none)
        let degree = if field != none { e.degree + " (" + field + ")" } else { e.degree }
        dated(s, e.period, [#text(font: s.fonts.heading, size: s.sizes.title, weight: "semibold", fill: s.primary)[#degree] \ #e.institution])
      }
    }
    ...
    if d.projects.len() > 0 {
      ...
      for item in d.projects {
        ...
        container(s, d, if role != none { item.name + " · " + role } else { item.name }, item.meta, item)
      }
    }
