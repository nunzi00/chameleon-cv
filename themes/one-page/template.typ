// Tema «one-page» de Chameleon CV (T-8.12, organización): todo en una página. Jerarquía tipográfica estricta, márgenes
// cortos, sin pie de página y recortes visibles: como mucho cinco puestos con cuatro logros cada uno, tres proyectos y
// tres logros destacados; lo que queda fuera se indica con «(+N)» tras el último elemento impreso. Los resúmenes de cada
// puesto y proyecto no se imprimen. Para candidaturas que exigen un folio.
...
// Para personalizar: `cv theme create mio --from one-page` y edita theme.toml o template.typ (contrato en
...
#let MAX-EXPERIENCE = 5
#let MAX-ACHIEVEMENTS = 4
#let MAX-PROJECTS = 3
#let MAX-HIGHLIGHTS = 3

#let take(xs, n) = xs.slice(0, calc.min(n, xs.len()))
// Marca del recorte: cuántos elementos quedan fuera (neutra en cualquier idioma).
#let more(s, xs, n) = if xs.len() > n { text(size: s.sizes.meta, fill: s.muted)[ (+#(xs.len() - n))] } else { [] }

#let cv(d, theme) = {
  let s = styles(theme)
  setup(s, d, footer: false, {
    header(s, d)

    if d.experience.len() > 0 {
      section(s, d.labels.experience)
      for item in take(d.experience, MAX-EXPERIENCE) {
        let location = item.at("location", default: none)
        block(sticky: true, above: 0.7em, below: 0.25em, grid(
          columns: (1fr, auto),
          column-gutter: 1em,
          align: (left, right),
          [#text(font: s.fonts.heading, size: s.sizes.title, weight: "semibold", fill: s.primary)[#item.role] · #item.company#if location != none [ #text(size: s.sizes.meta, fill: s.muted, style: "italic")[· #location]]],
          text(size: s.sizes.meta, fill: s.muted)[#item.period],
        ))
        let shown = take(item.achievements, MAX-ACHIEVEMENTS)
        for (i, a) in shown.enumerate() {
          achievement(s, a, tail: if i == shown.len() - 1 { more(s, item.achievements, MAX-ACHIEVEMENTS) } else { none })
        }
        if item.technologies != "" {
          block(above: 0.25em, text(size: s.sizes.meta, fill: s.muted)[#text(weight: "semibold")[#d.labels.technologies:] #item.technologies])
        }
      }
      if d.experience.len() > MAX-EXPERIENCE { block(above: 0.4em, text(size: s.sizes.meta, fill: s.muted)[#more(s, d.experience, MAX-EXPERIENCE)]) }
    }

    if d.projects.len() > 0 {
      section(s, d.labels.projects)
      for item in take(d.projects, MAX-PROJECTS) {
        let role = item.at("role", default: none)
        block(below: 0.3em, [#strong(item.name)#if role != none [ · #role] #text(size: s.sizes.meta, fill: s.muted)[· #item.meta#if item.technologies != "" [ · #item.technologies]]])
      }
      if d.projects.len() > MAX-PROJECTS { block(above: 0.2em, text(size: s.sizes.meta, fill: s.muted)[#more(s, d.projects, MAX-PROJECTS)]) }
    }
    ...
    if d.skillGroups.len() > 0 {
      ...
      grid(
        ...
        column-gutter: 1.4em,
        row-gutter: 0.3em,
        ..d.skillGroups.map(g => [#text(weight: "semibold")[#g.label:] #g.names]),
      )
    }

    if d.achievements.len() > 0 {
      section(s, d.labels.achievements)
      let shown = take(d.achievements, MAX-HIGHLIGHTS)
      for (i, a) in shown.enumerate() {
        achievement(s, a, tail: if i == shown.len() - 1 { more(s, d.achievements, MAX-HIGHLIGHTS) } else { none })
      }
    }

    if d.education.len() > 0 or d.certifications.len() > 0 {
      grid(
        columns: (1fr, 1fr),
        column-gutter: 1.4em,
        {
          if d.education.len() > 0 {
            ...
            for e in d.education {
              let field = e.at("field", default: none)
              let degree = if field != none { e.degree + " (" + field + ")" } else { e.degree }
              block(below: 0.3em, [#strong(degree) · #e.institution #text(size: s.sizes.meta, fill: s.muted)[· #e.period]])
            }
          }
        },
        {
          if d.certifications.len() > 0 {
            ...
            for c in d.certifications {
              let issuer = c.at("issuer", default: none)
              block(below: 0.3em, [#strong(c.name)#if issuer != none [ · #issuer] #text(size: s.sizes.meta, fill: s.muted)[· #c.date]])
            }
          }
        },
      )
