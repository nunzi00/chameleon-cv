// Tema «functional» de Chameleon CV (T-8.12, organización): CV funcional. Primero las competencias agrupadas por
// especialidad y los logros consolidados (los del perfil y los de cada puesto, con la empresa como origen); después
// los proyectos y la trayectoria en una línea por puesto. Para cambios de sector, huecos en la cronología o perfiles
// con muchos puestos cortos: se lee qué sabes hacer antes que dónde lo hiciste.
...
// Para personalizar: `cv theme create mio --from functional` y edita theme.toml o template.typ (contrato en
...
#let cv(d, theme) = {
  ...
  setup(s, d, {
    ...
    // 1. Competencias por especialidad, en dos columnas.
    if d.skillGroups.len() > 0 {
      ...
      grid(
        ...
        column-gutter: 1.6em,
        row-gutter: 0.7em,
        ..d.skillGroups.map(g => block(width: 100%, {
          block(below: 0.25em, text(font: s.fonts.heading, weight: "semibold", fill: s.primary)[#g.label])
          text(size: s.sizes.body)[#g.names]
        })),
      )
    }

    // 2. Logros: los del perfil y, a continuación, los de cada puesto con su empresa.
    let bullets = ()
    for a in d.achievements { bullets.push(achievement(s, a)) }
    for item in d.experience {
      for a in item.achievements { bullets.push(achievement(s, a, suffix: item.company)) }
    }
    if bullets.len() > 0 {
    ...
    // 3. Proyectos, una línea por proyecto.
    if d.projects.len() > 0 {
      section(s, d.labels.projects)
      for item in d.projects {
        let role = item.at("role", default: none)
        block(below: 0.45em, [#strong(item.name)#if role != none [ · #role] #text(size: s.sizes.meta, fill: s.muted)[· #item.meta#if item.technologies != "" [ · #item.technologies]]])
      }
    }

    // 4. Trayectoria: periodo, puesto y empresa, sin viñetas (los logros ya están arriba).
    if d.experience.len() > 0 {
      section(s, d.labels.experience)
      for item in d.experience {
        let location = item.at("location", default: none)
        block(below: 0.45em, grid(
          columns: (8.6em, 1fr),
          column-gutter: 1.1em,
          text(size: s.sizes.meta, fill: s.muted, weight: "semibold")[#item.period],
          [#strong(item.role) · #item.company#if location != none [ #text(size: s.sizes.meta, fill: s.muted, style: "italic")[· #location]]],
        ))
      }
    }
