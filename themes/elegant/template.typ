// Tema «elegant» de Chameleon CV (T-8.12, estilo): serif (Libertinus), nombre centrado en versalitas, doble
// filete fino bajo la cabecera, títulos de sección centrados entre filetes y metadatos en cursiva. Sobrio y editorial,
// para perfiles de dirección, consultoría, derecho o academia que no quieren color.
...
// Para personalizar: `cv theme create mio --from elegant` y edita theme.toml o template.typ (contrato en
...
#let head(s, d) = {
  align(center, {
    block(below: 0.35em, text(font: s.fonts.heading, size: s.sizes.name, weight: "regular", fill: s.primary, tracking: 0.03em, smallcaps(d.fullName)))
    let headline = d.at("headline", default: none)
...
// Título centrado entre dos filetes.
#let title(s, t) = block(
  width: 100%,
  above: 1.3em,
  below: 0.7em,
  ...
  grid(
    ...
    text(font: s.fonts.heading, size: s.sizes.section, weight: "regular", fill: s.primary, tracking: 0.08em, smallcaps(t)),
    line(length: 100%, stroke: 0.5pt + s.rule),
  ),
)
...
#let piece(s, d, heading, subtitle, item, location: none) = {
  block(sticky: true, above: 0.95em, below: 0.35em, grid(
    ...
    text(size: s.sizes.meta, style: "italic", fill: s.muted)[#subtitle],
  ))
