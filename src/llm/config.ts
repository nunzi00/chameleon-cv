...
export async function selectProvider(selection: ProviderSelection = {}, options: SelectProviderOptions = {}): Promise<ProviderSelectionResult> {
  ...
  const provider =
    entry.api === 'anthropic-messages'
      ...
      : createOpenAiCompatibleProvider({ id: entry.id, kind: 'remote', baseUrl, model, http, headers: { authorization: `Bearer ${key.key}` }, outputTokensFloor: entry.models.find((candidate) => candidate.id === model)?.outputTokensFloor, chatPath: entry.paths?.chat, modelsPath: entry.paths?.models, supportsSeed: entry.supportsSeed });
