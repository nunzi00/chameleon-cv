...
- **Salida** en Markdown (plantillas Handlebars) o PDF: pdfkit sin dependencias, o Typst con veintiún temas de calidad editorial —seis organizaciones del contenido (cronológica, funcional, híbrida, skills-first, portfolio de proyectos y una página) y quince estilos— o los tuyos.
...
| `cv improve apply <revisión>` | Aplica a tus fuentes las propuestas marcadas `[x]` en una revisión de `improve` o `summarize`: solo lo marcado, cambio mínimo, versión anterior completa guardada en `output/historial-fuentes/` y huella comprobada (si el original cambió, no escribe nada). La única orden, junto con `cv history restore`, que escribe en `data/sources/`. |
| `cv history` / `cv history show <entrada|latest> <ruta>` / `cv history restore <entrada|latest> <ruta>` | El histórico de versiones de las fuentes (T-8.10): lo que había antes de cada aplicación o restauración, entero; restaurar deja a su vez la versión actual en el histórico | `-d` · `--dry-run` · `--delete-review` |
...
| `cv llm models` | Catálogo de modelos locales (familia, razonamiento —ninguno, conmutable o siempre—, tamaño, RAM mínima, licencia, tareas recomendadas y espejo en Hugging Face) con lo que hay descargado en el Ollama configurado; `--json`. `cv llm up --model <id>` los descarga (con el espejo si el registro de Ollama falla; `--source huggingface` va directo) y `[llm] think = true` pide razonamiento a los que lo conmutan (T-8.13) |
...
- `cv improve apply` (y `cv history restore`) son las únicas órdenes que escriben en `data/sources/`: solo con tu marca `[x]`, tras guardar la versión anterior entera en `output/historial-fuentes/` (0600) y comprobar por huella que el original no cambió.
