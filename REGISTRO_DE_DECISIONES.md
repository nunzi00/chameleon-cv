...
| 2026-08-29 | Nueve decisiones de T-7.3 (arquitecturas, paquete, etiquetas, cadena de suministro, Trivy, Compose, publicación de la 1.1.0, UID/GID, visibilidad). | `amd64` + `arm64` en runners nativos con salvaguarda; un paquete con sufijo `-distroless`; `X.Y.Z`/`X.Y`/`X`/`latest`; SBOM + procedencia de BuildKit + atestación de GitHub, sin cosign; Trivy informativo en release y semanal; Compose con la versión exacta y prueba de coherencia; publicación retroactiva de la 1.1.0, **sustituida por el Director** al aprobar la entrega: la 1.1.0 no se publica y la primera imagen es la de la 1.1.1; UID/GID 1000; visibilidad pública comprobada por el Director. | `docs/ghcr-publication.md` §9 |
